...
namespace EncryptionDesktop.Services
{
    public class EncryptionService
    {
        ...
        public string Encrypt(string clearText, string saltText, string encryptionKey)
        {
            ...
            {
                byte[] keyBytes = Rfc2898DeriveBytes.Pbkdf2(encryptionKey, saltBytes, 10000, HashAlgorithmName.SHA256, 32);
                aes.Key = keyBytes;
                aes.IV = iv;
        ...
        public string Decrypt(string cipherText, string saltText, string decryptionKey)
        {
            ...
            using (Aes aes = Aes.Create())
            {
                byte[] keyBytes = Rfc2898DeriveBytes.Pbkdf2(decryptionKey, saltBytes, 10000, HashAlgorithmName.SHA256, 32);
                aes.Key = keyBytes;
                aes.IV = iv;
